...
> [!NOTE]
> **Experimental MCP Server:** The Interview Assistant integrates with an experimental standalone Node.js (NestJS) server (`apps/interview-mcp-server`) via the Model Context Protocol (MCP) using Server-Sent Events (SSE). It fetches mock frontend technical questions dynamically based on the user's difficulty level. Currently, the Next.js API route establishes a new SSE connection for every request; future improvements could explore persisting this connection or implementing a stateless HTTP transport to reduce overhead in serverless environments.
